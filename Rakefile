...
task :build => :clean do
	puts "===> Building ..."
  mkdir_p "target/logstash"
	mkdir_p "target/kibana-dashboards"
	sh "vendor/addon-common/bin/build.sh src/parsers/logsearch-for-weblogs.filters.conf.erb > target/logstash/logsearch-for-weblogs.filters.conf"
